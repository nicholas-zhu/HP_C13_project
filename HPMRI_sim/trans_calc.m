function ratio = trans_calc( A0, f0, mu, threshold)
...
pulse = [  p_de, zeros(size(g_diff)), p_de, p_inv, p_de, zeros(size(g_diff)), p_de, zeros(size(g_diff)), p_de, p_inv, p_de, zeros(size(g_diff)), 0];
gz = [p_de, g_diff, p_de, g_inv, p_de, g_diff, p_de, g_diff, p_de, g_inv, p_de, g_diff, 0];
g = zeros( 3, length( gz ));
g( 3, :) = gz;
...
mz_f = medfilt1(mz,8);
ratio = mean( mz_f < threshold ); % percentage of excitation
plot(l_z,mz_f);
xlabel('location/cm');
